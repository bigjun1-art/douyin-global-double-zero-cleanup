...
export function parseJsonPreservingLargeIntegers(text) {
  const scalarIds = String(text).replace(
    /(\"[A-Za-z0-9_]*(?:id|ids)\"\s*:\s*)(-?\d{16,})/gi,
    '$1"$2"',
  );
  const idArrays = scalarIds.replace(
    /(\"[A-Za-z0-9_]*(?:ids)\"\s*:\s*\[)([^\]]*)(\])/gi,
    (_, prefix, body, suffix) => `${prefix}${body.replace(/(?<![\"\d])-?\d{16,}(?![\"\d])/g, value => `"${value}"`)}${suffix}`,
  );
  return JSON.parse(idArrays);
}
...
function compactData(data) {
  ...
  const out = {};
  for (const key of ["project_id", "promotion_id", "project_ids", "promotion_ids", "page_info"]) {
...
async function requestJson({ baseUrl, token, operation, query, body, retryReads = 2 }) {
  ...
  for (let attempt = 0; attempt <= retryReads; attempt += 1) {
    try {
      ...
      let parsed;
      try { parsed = parseJsonPreservingLargeIntegers(text); } catch { throw new Error(`non-JSON response (HTTP ${response.status})`); }
      if (!response.ok) {
...
export async function runConfig(config, { execute = false, env = process.env } = {}) {
  ...
  for (const step of config.steps) {
    try {
      const response = await requestJson({ baseUrl, token, operation: step.operation, query: step.query, body: step.body });
      ...
      result.steps.push({ name: step.name, operation: step.operation, status: "verified", data: compactData(response.data) });
    } catch (error) {
...
export function selfTest() {
  ...
  const parsed = parseJsonPreservingLargeIntegers('{"material_id":9007199254740993,"project_ids":[9007199254740995],"stat_cost":12}');
  assert(parsed.material_id === "9007199254740993", "large material ID parsing failed");
  assert(parsed.project_ids[0] === "9007199254740995", "large ID array parsing failed");
  assert(parsed.stat_cost === 12, "ordinary numeric parsing changed");
...
async function main() {
  ...
  const config = parseJsonPreservingLargeIntegers(fs.readFileSync(configPath, "utf8"));
