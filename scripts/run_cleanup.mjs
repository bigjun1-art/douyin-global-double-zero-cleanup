...
function parseArgs(argv) {
  ...
  if (out.rule === "double-zero" && out.commitLedger) fail("DOUBLE_ZERO_LEDGER_FORBIDDEN");
...
function runSelfTest() {
  ...
  if (!shouldDeleteForFixedRule(0, 0, false) || shouldDeleteForFixedRule(5, 0, true)) fail("self-test double-zero rule isolation failed");
  console.log(JSON.stringify({ ok: true, rule: FIXED_RULE, tests: ["numeric-metrics", "surface-pt-lock", "double-zero-rule-isolation", "explicit-execute-gate", "delayed-readback-contract"] }));
}
...
const ledger = cfg.rule === "double-zero" ? {} : readLedger(cfg.ledger);
