...
`douyin-global-double-zero-cleanup` 是面向 Codex 的抖音本地生活投放自动化 Skill。仅删除消耗为 0 且产出为 0 的全域单条视频，默认预演，执行前确认计划和数量，删除后回读。
...
将本仓库目录复制到 Codex 的 Skill 目录，目录名保持为 `douyin-global-double-zero-cleanup`。参数和执行边界见 [SKILL.md](SKILL.md)。
